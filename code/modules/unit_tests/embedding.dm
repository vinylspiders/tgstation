...
/datum/unit_test/embedding/Run()
	...
	sleep(0.1 SECONDS) // Projectiles process-- we should be sleep to ensure that the bullet has time to land before checking for the embed.
	var/list/components = victim.GetComponents(/datum/component/embedded)
	TEST_ASSERT_EQUAL(components.len, 1, "Projectile with 100% embed chance " + (components.len == 0 ? "didn't embed" : "embedded multiple times"))
